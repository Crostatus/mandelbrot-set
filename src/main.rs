...
fn main() {
    ...
    crossbeam::scope(|spawner| {
        for (i, band) in bands.into_iter().enumerate() {
            ...
            let band_upper_left = pixel_to_complex_plain(bounds, (0, top), upper_left, lower_right);
            let band_lower_right =
                pixel_to_complex_plain(bounds, (bounds.0, top + height), upper_left, lower_right);
...
fn write_image(
    ...
    let _output = File::create(filename)?;
